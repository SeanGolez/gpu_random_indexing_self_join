...
bool foundMax=0;
	...
	indexes[x]=loopRng[x];	
		...
		evaluateCell(nCells, indexes, gridCellLookupArr, nNonEmptyCells, database, epsilon, index, indexLookupArr, point, cnt, keyValPairs, pointIdx, false, nDCellIDs, workCounts, &localCnt);
...
#else
	for (int k=index[GridIndex].indexmin; k<=index[GridIndex].indexmax; k++){
		evalPoint(indexLookupArr, k, database, epsilon, point, cnt, keyValPairs, pointIdx, differentCell, localCnt);
#if COUNTMETRICS == 1
			atomicAdd(&workCounts[0],1);
#endif
        }
